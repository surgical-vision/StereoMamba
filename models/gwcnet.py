...
class GwcNet(nn.Module):
    ...
    def forward(self, features_left, features_right):
        ...
        if self.training:
            ...
            cost0 = F.interpolate(cost0, [self.maxdisp, features_left[0].size()[2], features_left[0].size()[3]], mode='trilinear')
            cost0 = torch.squeeze(cost0, 1)
            pred0 = F.softmax(cost0, dim=1)
            pred0 = disparity_regression(pred0, self.maxdisp)

            cost1 = F.interpolate(cost1, [self.maxdisp, features_left[0].size()[2], features_left[0].size()[3]], mode='trilinear')
            cost1 = torch.squeeze(cost1, 1)
            pred1 = F.softmax(cost1, dim=1)
            pred1 = disparity_regression(pred1, self.maxdisp)

            cost2 = F.interpolate(cost2, [self.maxdisp, features_left[0].size()[2], features_left[0].size()[3]], mode='trilinear')
            cost2 = torch.squeeze(cost2, 1)
            pred2 = F.softmax(cost2, dim=1)
            pred2 = disparity_regression(pred2, self.maxdisp)

            cost3 = F.interpolate(cost3, [self.maxdisp, features_left[0].size()[2], features_left[0].size()[3]], mode='trilinear')
