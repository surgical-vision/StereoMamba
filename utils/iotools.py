...
def save_disp(out_path, disparity, size=None, scale_factor=1):
    ...
    disp_np=disparity[0].detach().cpu().numpy()

    if size is not None:
        disp_np = disp_np[:size[0],:size[1]]
    scale_factor_np = np.array(float(scale_factor))
    #multipling by 256 and saving as a uint16 png we keep subpixel information
    out_img = Image.fromarray((disp_np*scale_factor_np).astype(np.uint16))
    out_img.save(out_path)
    # plt.imsave(out_path, out_img, cmap='jet')
    # print('saved')
...
class NormalizeInverse:
    ...
    def __call__(self, tensor):
        ...
        # Clamp to ensure valid image values between 0 and 1
        tensor.clamp_(0, 1)
        return tensor